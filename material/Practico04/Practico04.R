setwd("/home/mlopez/git/metodos_estadisticos_bayesianos/material/Practico04/")
...
# importar datos
DF=read.csv("lechuzasPr04.csv", header = T,stringsAsFactors=T)
str(DF)
summary(DF)
...
# Distribucion de prob de la var de respuesta: SiblingNegotiation
poiss=fitdist(DF$SiblingNegotiation,"pois")
negbin=fitdist(DF$SiblingNegotiation,"nbinom")
CDF=cdfcomp(list(poiss,negbin),addlegend=T,legendtext=c("Poisson","NegBin"),
               plotstyle = "ggplot")+
...
QQ=qqcomp(list(poiss,negbin),addlegend=F,plotstyle = "ggplot")+
  theme_bw()+
